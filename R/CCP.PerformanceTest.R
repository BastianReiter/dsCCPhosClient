...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
CCP.PerformanceTest <- function(ServerSpecifications,
                                ScenarioA.Runs = 1,
...
#-------------------------------------------------------------------------------
                        ...
                        SingleRun.DSConnections)
  {
      ...
      CurationMessages <- DSI::datashield.aggregate(conns = SingleRun.DSConnections,
                                                    expr = call("GetReportingObjectDS",
                                                                ObjectName.S = "Messages"),
                                                    async = DS.async)

      CurationDurations <- CurationMessages %>%
                                map(\(X) as.numeric(X$Process.Duration)) %>%
                                tibble::enframe(name = "Server",
                                                value = "Duration.Curation")

      Tracker <- Tracker %>%
                      mutate(Duration.Curation.Sum = sum(as.numeric(CurationDurations$Duration.Curation)),
                             Duration.Curation.Mean = mean(as.numeric(CurationDurations$Duration.Curation)),
                             Duration.Curation.SD = sd(as.numeric(CurationDurations$Duration.Curation)),
                             Duration.Curation.Median = median(as.numeric(CurationDurations$Duration.Curation)))

      return(list(Tracker = Tracker,
                  ServerSpecificCurationDurations = CurationDurations))
  }
...
#===============================================================================
  ...
  if (ScenarioA.Runs > 0)
  {
      for (i in 1:ScenarioA.Runs)
      {
          ...
          for (CurrentSampleSize in ScenarioA.SampleSizes)
          {
              ...
              Tracker.ScenarioA <- Tracker.ScenarioA %>%
                                        bind_rows(SingleRunResults$Tracker)

              Tracker.ScenarioA.ServerSpecific <- Tracker.ScenarioA.ServerSpecific %>%
                                                      bind_rows(SingleRunResults$ServerSpecificCurationDurations)
          }
      ...
      Results.A <- Results.A %>%
                        mutate(Duration.Login = as.numeric(Time.AfterLogin - Time.StartRun),
                               Duration.Loading = as.numeric(Time.AfterLoading - Time.AfterLogin),
                               Duration.SampleDrawing = as.numeric(Time.AfterSampleDrawing - Time.AfterLoading),
                               Duration.Curation.WithTI = as.numeric(Time.AfterCuration - Time.AfterSampleDrawing),
                               Duration.Curation.WithoutTI = Duration.Curation.Sum,
  ...
  if (ScenarioB.Runs > 0)
  {
      for (i in 1:ScenarioB.Runs)
      {
          for (k in ScenarioB.SequenceServerCount)
          {
              ...
              # Connect to CCP servers
              ScenarioB.CCPConnections <- ConnectToCCP(ServerSpecifications = CurrentServerSpecifications)
              ...
              Results.B.CurrentRun <- Results.B.CurrentRun %>%
                                          bind_cols(SingleRunResults$Tracker)

              Results.B <- Results.B %>%
                                bind_rows(Results.B.CurrentRun)

              Results.B.ServerSpecific <- Results.B.ServerSpecific %>%
                                              bind_cols(SingleRunResults$ServerSpecificCurationDurations)
